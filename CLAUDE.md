...
- **Measurement ID** is `G-0GNHHLGV51` (GA4 property owned by `connect@avantamour.in`, created 11 Sep 2026 with the paid-ads agency), hardcoded in **two** places inside the BaseLayout `<head>`: the `<script async src="https://www.googletagmanager.com/gtag/js?id=…">` loader and the `gtag('config', '…', { anonymize_ip: true })` call. If it ever changes, update both — there is no constant for it. The previous property `G-R9K61CYZV3` (data up to 12 Sep 2026) is orphaned — nobody has confirmed admin access to it. Do not reinstate it alongside the new one.
